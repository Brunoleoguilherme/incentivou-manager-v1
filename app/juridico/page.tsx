...
import {
  ...
  Scale, FileSignature, FileText, ShieldCheck, Pencil, Trash2,
  X, Save, Calendar, Building2, UserRound, AlertTriangle,
  CheckCircle2, Clock, Eye, Download, BadgeCheck, Gavel,
} from 'lucide-react';

const STORAGE_KEY = 'incentivou_juridico';
...
const iniciais = [
  {
    id: '1',
    titulo: 'Contrato de Patrocínio Incentivado',
    tipo: 'Contrato de patrocínio',
    parte: 'Empresa Apoio Brasil',
    projeto: 'Esporte para Todos',
    responsavel: 'Jurídico',
    status: 'Assinado',
    vencimento: '2026-12-31',
    valor: 420000,
    observacoes: 'Contrato assinado e vinculado ao projeto aprovado.',
  },
  {
    id: '2',
    titulo: 'Contrato de Fornecedor de Materiais',
    tipo: 'Contrato de fornecedor',
    parte: 'Fornecedor Alpha',
    projeto: 'Formação Olímpica Comunitária',
    responsavel: 'Operações',
    status: 'Pendente assinatura',
    vencimento: '2026-06-25',
    valor: 85000,
    observacoes: 'Aguardando assinatura digital do fornecedor.',
  },
  {
    id: '3',
    titulo: 'Termo LGPD de Beneficiários',
    tipo: 'Termo LGPD',
    parte: 'Beneficiários do projeto',
    projeto: 'Flag Football nas Escolas',
    responsavel: 'Compliance',
    status: 'Vencendo',
    vencimento: '2026-06-10',
    valor: 0,
    observacoes: 'Revisar validade dos consentimentos e política de tratamento de dados.',
  },
];
...
export default function JuridicoPage() {
  ...
  const filtrados = useMemo(() => {
    return dados.filter((item) => {
      const texto = `${item.titulo} ${item.tipo} ${item.parte} ${item.projeto} ${item.responsavel} ${item.status}`.toLowerCase();
  ...
  }, [dados, busca, filtroStatus, filtroTipo]);

  const metricas = useMemo(() => ({
    contratos: dados.length,
    assinados: dados.filter((p) => p.status === 'Assinado').length,
    pendentes: dados.filter((p) => p.status === 'Pendente assinatura' || p.status === 'Em revisão').length,
    vencendo: dados.filter((p) => p.status === 'Vencendo' || p.status === 'Vencido').length,
    valorContratado: dados.reduce((acc, p) => acc + Number(p.valor || 0), 0),
  }), [dados]);
  ...
  function salvar(e) {
    ...
    if (!form.titulo || !form.parte) {
      alert('Preencha o título do documento e a parte envolvida.');
      return;
    }
  ...
  function excluir(id) {
    if (!confirm('Deseja excluir este documento jurídico?')) return;
    ...
    <main className="page">
      ...
      <section className="hero">
        <div>
          ...
          <span>Contratos, LGPD e compliance</span>
          <h1>Jurídico e Compliance</h1>
          <p>
            Gerencie contratos, termos, procurações, LGPD, autorizações de imagem,
            assinaturas digitais, vencimentos e histórico jurídico dos projetos.
          </p>
          ...
          <div className="actions">
            ...
            <button type="button" className="secondary" onClick={() => alert('Fluxo de assinatura digital será integrado na próxima etapa.')}>
              <FileSignature size={18} />
              Assinatura digital
            </button>

            <button type="button" className="secondary" onClick={() => alert('Relatório jurídico consolidado será gerado na próxima etapa.')}>
              <Gavel size={18} />
              Relatório jurídico
            </button>
          </div>
        ...
        <div className="heroPanel">
          <div className="panelItem">
            <Scale size={24} />
            <strong>{metricas.contratos}</strong>
            <span>Documentos jurídicos</span>
          </div>

          <div className="panelItem">
            <CheckCircle2 size={24} />
            <strong>{metricas.assinados}</strong>
            <span>Documentos assinados</span>
          </div>

          <div className="panelItem">
            <AlertTriangle size={24} />
            <strong>{metricas.vencendo}</strong>
            <span>Vencendo ou vencidos</span>
          </div>
        </div>
      </section>
      ...
      <section className="metricsGrid">
        <div className="metricCard" style={{ "--accent": "#24E49B" } as React.CSSProperties}>
          <div><span>Contratos</span><strong>{metricas.contratos}</strong></div>
          <FileText size={30} />
        </div>

        <div className="metricCard" style={{ "--accent": "#22C55E" } as React.CSSProperties}>
          <div><span>Assinados</span><strong>{metricas.assinados}</strong></div>
          <BadgeCheck size={30} />
        </div>
        ...
        <div className="metricCard" style={{ "--accent": "#F43F5E" } as React.CSSProperties}>
          <div><span>Vencendo</span><strong>{metricas.vencendo}</strong></div>
      ...
      <section className="listCard">
        <div className="listHeader">
          <div>
            ...
            <p>Pesquise, filtre e acompanhe documentos, vencimentos, assinaturas e compliance.</p>
          </div>
        ...
        <div className="projectGrid">
          {filtrados.map((item) => {
            const cor =
              item.status === 'Assinado' ? '#22C55E' :
              item.status === 'Vencendo' || item.status === 'Vencido' ? '#F43F5E' :
              item.status === 'Pendente assinatura' ? '#FB923C' :
              item.status === 'Em revisão' ? '#A855F7' :
              '#24E49B';
                ...
                <div className="projectInner">
                  <div className="projectTop">
                    ...
                    <div className="projectActions">
                      ...
                      <button type="button" onClick={() => baixarDocumento(item)}>
                        <Download size={17} />
                      </button>
                  ...
                  <div className="infoList">
                    <span><Building2 size={15} /> Parte: {item.parte || '-'}</span>
                    <span><FileText size={15} /> Projeto: {item.projeto || '-'}</span>
                    <span><UserRound size={15} /> Responsável: {item.responsavel || '-'}</span>
                    <span><Calendar size={15} /> Vencimento: {dataBR(item.vencimento)}</span>
                    <span><ShieldCheck size={15} /> LGPD / auditoria: monitorado</span>
                    <span><Gavel size={15} /> Valor vinculado: {moeda(item.valor)}</span>
                  </div>
                </div>
              </div>
            );
          ...
          {filtrados.length === 0 && (
            <div className="empty">
              Nenhum documento jurídico encontrado com os filtros selecionados.
            </div>
          )}
        </div>
      </section>
        ...
        <div className="modalOverlay">
          <form className="modal" onSubmit={salvar}>
            <div className="modalHeader">
              <div>
                <span>{editandoId ? 'Editar documento' : 'Novo documento jurídico'}</span>
                <h2>{editandoId ? 'Atualizar jurídico' : 'Cadastrar documento'}</h2>
              </div>
            ...
            <div className="formGrid">
              ...
              <label>
                Parte envolvida *
                <input value={form.parte} onChange={(e) => setForm({ ...form, parte: e.target.value })} />
              </label>
